...
const OtherItemAndQunatity = ({ onChange, value, otherItemsArr }) => { 



  const returnItemName = (item) => {

    if(!item || otherItemsArr.length<=0){
        console.log(item)
        return 
    }
    
    const otherProductItem = otherItemsArr.filter((otheritem)=>otheritem.id===item.other_products_id)[0]
    return `${otherProductItem['name']}`

  }
  console.log(otherItemsArr)
...
return (
      ...
      <div className="table-header-row">
        <div className="table-header-cell" style={{minWidth:"30rem"}}>Other products</div>
      ...
      {value.map((item, index)=> <div key={index} className="table-row">
        ...
        <div className="table-row-cell" style={{paddingLeft:"1px", width:"auto", paddingRight:"1px", background:"rgba(242, 241, 238, 0.6)", marginBottom:"0.6px"}}>
        <Stextfield
            key={index}
            name={`${index}`}
            label=""
...
OtherItemAndQunatity.propTypes = {
...
export default OtherItemAndQunatity;