  ...
  useEffect(() => {
    ...
    if(selectedEmployeePayment){
      ...
      reset({
        ...
        paidOptionInfo: selectedEmployeePayment.paidOptionInfo,
...
console.log(errors, selectedEmployeePayment)   
  ...
  return (
    ...
    <form  className='formclass' onSubmit={handleSubmit(data=>selectedEmployeePayment?UpdateEmployeePayment({data, Paymentid:`${selectedEmployeePayment.id}`}):createNewEmployeePayment(data))}>
       <div style={{ flexDirection: "column", display: "flex" , width:"fit-content", margin:"0px 20px", marginTop:"0", marginBottom:"70px"}}>
        <div className='formheading'>{selectedEmployeePayment?'Update payment': 'Add payment'}</div>
