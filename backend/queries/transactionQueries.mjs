...
export const transactionFilteredQry = (previousCursor, nextCursor) => { 
    return `
    SELECT * FROM (
        SELECT
    ...
    WHERE
        (${nextCursor ? `row_num > ${nextCursor}` : 'true'}) AND
        (${previousCursor ? `row_num < ${previousCursor}` : 'true'})
    ORDER BY payment_date DESC
    LIMIT $10;
    `;
};
...
export const transactionFilteredSummaryQry = 
...
SELECT
    ...
    SUM(CASE WHEN category = 'Customer payments' THEN COALESCE((payment_info->>'cash')::numeric, 0) ELSE 0 END) AS positive_total_cash,
    SUM(CASE WHEN category = 'Customer payments' THEN COALESCE((payment_info->>'upi')::numeric, 0) ELSE 0 END) AS positive_total_upi,
    SUM(CASE WHEN category = 'Customer payments' THEN COALESCE((payment_info->>'cheque')::numeric, 0) ELSE 0 END) AS positive_total_cheque,
    SUM(CASE WHEN category = 'Customer payments' THEN COALESCE((payment_info->>'other')::numeric, 0) ELSE 0 END) AS positive_total_other,
    SUM(CASE WHEN category != 'Customer payments' THEN COALESCE((payment_info->>'cash')::numeric, 0) ELSE 0 END) AS negative_total_cash,
    SUM(CASE WHEN category != 'Customer payments' THEN COALESCE((payment_info->>'upi')::numeric, 0) ELSE 0 END) AS negative_total_upi,
    SUM(CASE WHEN category != 'Customer payments' THEN COALESCE((payment_info->>'cheque')::numeric, 0) ELSE 0 END) AS negative_total_cheque,
    SUM(CASE WHEN category != 'Customer payments' THEN COALESCE((payment_info->>'other')::numeric, 0) ELSE 0 END) AS negative_total_other,
    SUM(CASE WHEN category = 'Vendor payments' THEN COALESCE(debit::numeric, 0) ELSE 0 END) AS sum_vendor_payments
