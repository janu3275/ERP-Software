  ...
  addMarketplace: asyncHandler(async (req, res, next) => {
    ...
    try {
      const newMarketplace = (
        await queryDB(addMarketplaceQuery, [name, image_src, companyid])
      ).rows[0];
      res
        .status(201)
        .json({
      ...
      return res
        ...
        .json({
          success: false,
          message: "Error adding marketplace",
          error: error,
        });
    }
  }),
...
`;
    ...
    try {
      const updatedMarketplace = (
        await queryDB(updateMarketplaceQuery, [name, image_src, id])
      ).rows[0];
  ...
  deleteMarketplace: asyncHandler(async (req, res, next) => {
    ...
    const deleteMarketplaceQuery = `
           ...
           RETURNING *;`;
    try {
      const deletedMarketplace = (
        await queryDB(deleteMarketplaceQuery, [market_id])
      ).rows[0];
  ...
  getAllMarketplaces: asyncHandler(async (req, res, next) => {
    ...
    try {
      const allMarketplaces = (
        await queryDB(getAllMarketplacesQuery, [companyid])
      ).rows;
      res
        .status(200)
        .json({
      ...
      return res
        ...
        .json({
          success: false,
          message: "Error retrieving marketplaces",
          error: error,
        });
    }
  }),
  selectMarketplace: asyncHandler(async (req, res, next) => {
    ...
    try {
      const Marketplace = (
        await queryDB(getAllMarketplacesQuery, [companyid, id, name])
      ).rows[0];
      ...
      return res
        ...
        .json({
          ...
          data: { token, marketplace: name, image_src },
        });
    } catch (error) {
      return res
        .status(500)
