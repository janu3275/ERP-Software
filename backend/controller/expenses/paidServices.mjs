...
const paymentForServicesApi = {

  addPaymentForServices: asyncHandler(async (req, res, next) => {
    // BODY VALIDATION
    const schema = Joi.object({
        
      expense_service_types_id: Joi.number().integer().required(),
    ...
    const addPaymentQuery = `
            INSERT INTO payment_for_services_used (expense_service_types_id, attachment_path, payment_date, cash, upi, cheque, other, note)
    ...
    try {
      ...
      const addedPayment = (
        await queryDB(addPaymentQuery, [
          value.expense_service_types_id,
          itemPath,
          value.payment_date,
          cash,
          upi,
  ...
  updatePaymentForServices: asyncHandler(async (req, res, next) => {
    // BODY VALIDATION
    const schema = Joi.object({
    ...
    const updatePaymentQuery = `
            UPDATE payment_for_services_used
            SET expense_service_types_id = $1, attachment_path = $2, payment_date = $3, cash = $4, upi = $5, cheque = $6, other = $7, note = $8
            WHERE id = $9
            RETURNING *;`;
    ...
    try {
      ...
      const prevPath = (
        await queryDB(`SELECT attachment_path FROM payment_for_services_used WHERE id = $1`, [value.payment_id])
  ...
  deletePaymentForServices: asyncHandler(async (req, res, next) => {
    // BODY VALIDATION
    const schema = Joi.object({
      payment_id: Joi.number().integer().required(),
    });
    ...
    const deletePaymentQuery = `
            DELETE FROM payment_for_services_used WHERE id = $1
            RETURNING *;`;
  ...
  getAllPaymentsForServices: asyncHandler(async (req, res, next) => {
    ...
    const getPaymentsQuery = `
            SELECT
                pe.id AS id,
                est.service_name as service_name,
                pe.expense_service_types_id,
                pe.attachment_path,
                pe.payment_date,
                ...
                pe.note
            FROM
                payment_for_services_used AS pe
            JOIN
                expense_service_types AS est
            ON
                pe.expense_service_types_id = est.id
            WHERE
                est.market_id = $1;`;
...
const getFilteredPaymentsforServices = `SELECT
pe.id AS id,
est.service_name AS service_name,
pe.expense_service_types_id,
pe.attachment_path,
